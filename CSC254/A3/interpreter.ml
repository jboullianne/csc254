...
(* Used to check implementation ~ should print a 10x10 multiplication table *)
let test_prog = "
                x := 1
                while x <= 10
                  y := 1
...
(* properly subtracts two value types *)
...
(* properly multiplys two value types *)
...
(* properly divides two value types *)
...
and interpret_sl (sl:ast_sl) (mem:memory)
  ...
  (* your code should replace the following line *)
  match sl with
  | stmt :: stmt_list -> (start_sl stmt_list (interpret_s stmt mem inp outp))
  | [stmt] -> interpret_s stmt mem inp outp
  | _ -> (true, mem, inp, outp)
...
let test_parse_tree = parse ecg_parse_table test_prog;;
let test_syntax_tree = ast_ize_P test_parse_tree;;
let test_interpret = interpret test_syntax_tree "";;
